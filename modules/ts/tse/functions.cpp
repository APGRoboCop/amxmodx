...
static cell AMX_NATIVE_CALL tse_setuserstamina(AMX *amx, cell *params) 
{
	...
	MESSAGE_BEGIN(MSG_ONE, GetMsgIDByName("KFuPower"), nullptr, Player(pid)->PlayerEdict);
		WRITE_BYTE(static_cast<byte>(round(amx_ctof(params[2]))));
	MESSAGE_END();
	return 1;
}
...
static cell AMX_NATIVE_CALL tse_giveuserpwup(AMX *amx, cell *params)
{
	const byte pid = params[1];
	CHECK_PLAYER_ZERO(pid)
	const uint16_t type = params[2];
	if (type < 1 || type > 256) return 0;
	const byte duration = params[3];
...
static cell AMX_NATIVE_CALL tse_setfakepwup(AMX *amx, cell *params)
{
	const byte pid = params[1];
	CHECK_PLAYER_ZERO(pid)
	const byte pwup = params[2];
	const float duration = amx_ctof(params[3]);
	...
	if (pwup == TSE_PWUP_SLOWMO || pwup == TSE_PWUP_SLOWPAUSE) 
	{
		...
		const float auradist = amx_ctof(params[5]);
...
static cell AMX_NATIVE_CALL tse_sendweapparams(AMX *amx, cell *params)
{
	...
	MESSAGE_BEGIN(MSG_ONE, GetMsgIDByName("CustomWP"), nullptr, Player(pid)->PlayerEdict);
...
static cell AMX_NATIVE_CALL tse_getusercurrweap(AMX *amx, cell *params)
{
	const byte pid = params[1];
	CHECK_PLAYER_NGTV(pid)
	const int weapon = Player(pid)->CurrentWeapon;
	return weapon;
}
...
static cell AMX_NATIVE_CALL tse_setweapfiremode(AMX *amx, cell *params)
{
	const byte pid = params[1];
	CHECK_PLAYER_ZERO(pid)
	const byte weapon = params[2];
	const byte mode = params[3];
	if (weapon < 1 || weapon > MAXWEAPS) return 0;
	if (mode < 0 || mode > 5) return 0;
	if (!WeaponsList[weapon].offsets.clip) return 0;
	const uint32_t ofstval = 1 + WeaponsList[weapon].offsets.fmbase + FireModes[mode];
...
static cell AMX_NATIVE_CALL tse_setweapatcments(AMX *amx, cell *params)
{
	...
	CHECK_PLAYER_ZERO(pid)
	const byte weapon = params[2];
	const byte attachments = params[3];
	const byte isactive = params[4];
	if (weapon < 1 || weapon > MAXWEAPS) return 0;
	...
	if (Player(pid)->CurrentWeapon == weapon)
	{
		...
		Player(pid)->UpdateHUD();
		if (isactive) {
			MESSAGE_BEGIN(MSG_ONE, GetMsgIDByName("ActItems"), nullptr, Player(pid)->PlayerEdict);
				WRITE_BYTE(1);
...
static cell AMX_NATIVE_CALL tse_setweapready(AMX *amx, cell *params)
{
	...
	const byte weapon = params[2];
	if (!WeaponsList[weapon].offsets.clip) return 0;
	const int isready = params[3];
	Player(pid)->SetWeapPDataInt(WeaponsList[weapon].offsets.clip + 8, static_cast<int>(isready) == 0);
	return 1;
}
...
static cell AMX_NATIVE_CALL tse_isuserhasweap(AMX *amx, cell *params)
{
	...
	const byte weapon = params[2];
	if (!WeaponsList[weapon].offsets.clip) return -1;
	const byte offset = WeaponsList[weapon].offsets.fmbase != 0 ? 1 : 0;
...
{
	const byte weapid = params[1];
	cell *coordptr = MF_GetAmxAddr(amx, params[2]);
	const uint16_t clips = params[3];
	const byte atcments = params[4];
	const short ttl = params[5];
