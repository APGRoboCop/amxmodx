...
typedef struct MMDB_search_node_s {
    ...
    extern MMDB_lookup_result_s MMDB_lookup_string(MMDB_s * mmdb,
                                                   const char * ipstr,
                                                   int * gai_error,
                                                   int * mmdb_error);
    extern MMDB_lookup_result_s MMDB_lookup_sockaddr(
               ...
               int * mmdb_error);
    extern int MMDB_read_node(MMDB_s * mmdb, uint32_t node_number,
                              MMDB_search_node_s * node);
    extern int MMDB_get_value(MMDB_entry_s * start,
                              MMDB_entry_data_s * entry_data,
                              ...);
    extern int MMDB_vget_value(MMDB_entry_s * start,
                               MMDB_entry_data_s * entry_data,
                               va_list va_path);
    ...
    extern int MMDB_get_entry_data_list(
               MMDB_entry_s *start, MMDB_entry_data_list_s ** entry_data_list);
